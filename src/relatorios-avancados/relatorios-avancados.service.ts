...
export class RelatoriosAvancadosService {
  constructor(
    ...
    private clientesRepo: Repository<Customer>,
  ) {}

  private buildDateFilter(dataInicio?: string, dataFim?: string) {
    if (dataInicio && dataFim) {
      return Between(new Date(dataInicio), new Date(dataFim));
    }
    if (dataInicio) {
      return Between(new Date(dataInicio), new Date());
    }
    if (dataFim) {
      return Between(new Date('2000-01-01'), new Date(dataFim));
    }
    return undefined;
  }
  ...
  async getResumoGeral(usuario: string, filtros: RelatorioFiltrosDto) {
    ...
    // Despesas (apenas se tipo for despesa ou ambos)
